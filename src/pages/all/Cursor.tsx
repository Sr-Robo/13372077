...
type Particle = {
  ...
  born: number; life: number; size: number; rgb: string;
};
...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    if (!canvas || window.matchMedia('(prefers-reduced-motion: reduce), (pointer: coarse)').matches ||
        'ontouchstart' in window || navigator.maxTouchPoints > 0) return undefined;
  ...
  return <canvas ref={canvasRef} className="cpk-quantum-cursor" aria-hidden="true" />;
}
